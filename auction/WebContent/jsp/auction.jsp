	...
	<div class="container">
		...
		<div class="auction_info_panel">
			...
			<c:if test="${auction != null}">
				<div class="col-md-12">
					...
					<div class="col-md-7 col-md-offset-1 text-left auction_lot">
						...
						<form action="FrontController" method="GET">
							...
							<button type="submit" class="btn">${auction.lot.name }</button>
						</form>
					</div>
					<div class="col-md-7 col-md-offset-1 text-left auction_lot_type">
					...
					<div class="col-md-7 col-md-offset-1 text-left auction_lot_quantity">
					...
					<div class="col-md-7 col-md-offset-1 text-left auction_status">
						...
						<c:if test="${auction.status == 'ACTIVE' }">
							<fmt:message bundle="${current_locale}" key="locale.auction.status.active"/>
						</c:if>
						<c:if test="${auction.status == 'COMPLETED' }">
							<fmt:message bundle="${current_locale}" key="locale.auction.status.completed"/>
						</c:if>
						<c:if test="${auction.status == 'PENDING_PAYMENT' }">
							<fmt:message bundle="${current_locale}" key="locale.auction.status.pending.payment"/>
						</c:if>
					</div>
					<div class="col-md-7 col-md-offset-1 text-left auction_type">
						...
						<c:if test="${auction.type == 'ENGLISH' }">
							<fmt:message bundle="${current_locale}" key="locale.auction.type.english"/>
						</c:if>
						<c:if test="${auction.type == 'ONLINE' }">
							<fmt:message bundle="${current_locale}" key="locale.auction.type.online"/>
						</c:if>
					</div>
					<div class="col-md-7 col-md-offset-1 text-left auction_start_time">
					...
					<c:if test="${auction.type == 'ONLINE' }">
						<div class="col-md-7 col-md-offset-1 text-left auction_end_time">
							<span class="auction_info_text"><fmt:message bundle="${current_locale}" key="locale.auction.end.time"/>:</span>
							<span>${auction.endTime }</span>
						</div>
					</c:if>
					<div class="col-md-7 col-md-offset-1 text-left auction_min_bet">
					...
					<c:if test="${not empty auction.lastBetUser }">
						<div class="col-md-7 col-md-offset-1 text-left auction_last_bet_user">
							<span class="auction_info_text"><fmt:message bundle="${current_locale}" key="locale.auction.last.bet.user"/>:</span>
							<form action="FrontController" method="GET">
								<input type="hidden" name="command" value="GET_USER_INFO"/>
								<input type="hidden" name="userLogin" value="${auction.lastBetUser}"/>
								<button type="submit" class="btn"><span>${auction.lastBetUser }</span></button>
							</form>
							<c:if test="${auction.status == 'ACTIVE' }">
								<div class="col-md-7 col-md-offset-1 text-left auction_current_bet">
							...
							<div class="col-md-7 col-md-offset-1 text-left auction_current_bet_time">
				...
				<div class="col-md-12 bet_pay_panel">
					<c:if test="${auction.status == 'ACTIVE' }">
						<form action="FrontController" method="POST" onsubmit="validate(); return false">
