...
<head>
	...
	<title><fmt:message bundle="${current_locale}" key="locale.offer.lot"/></title>
</head>
<body>
	<c:import url="WEB-INF/jsp/header.jsp" />
	<div class="container">
		<div class="col-md-12 lot_offer_panel">
			<div class="row">
				<c:if test="${requestScope.isLotDataInvalid == true}">
					<div id="lot_offer_info_invalid_message">
						<p><fmt:message bundle="${current_locale}" key="locale.lot.info.invalid"/></p>
					</div>
				</c:if>
				<c:if test="${requestScope.isLotDataInvalid != true}">
					<div id="lot_offer_info_invalid_message" style="display:none;">
						<p><fmt:message bundle="${current_locale}" key="locale.lot.info.invalid"/></p>
					</div>
				</c:if>
				<div class="lot_offer_form col-md-6 col-md-offset-2">
					<form action="FrontController" method="POST">
						<input type="hidden" name="command" value="OFFER_LOT"/>
						<p class="lot_offer_form_heading col-md-offset-2 text-center"><fmt:message bundle="${current_locale}" key="locale.offer.lot"/></p>
						...
						<div class="row">
							<div class="col-md-4 col-md-offset-5">
								<button class="btn btn-lg btn-primary btn-block" type="submit"><fmt:message bundle="${current_locale}" key="locale.offer.lot.button"/></button>            
						 	</div>
						</div>
					</form>
				</div>
