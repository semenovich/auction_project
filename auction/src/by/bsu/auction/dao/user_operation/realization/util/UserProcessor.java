...
public final class UserProcessor {
	...
	private static final String GET_USERS_BY_SEARCHING_SQL_STATEMENT = "SELECT u.su_login AS userLogin, u.su_surname AS userSurname, u.su_Name AS userName, u.su_password AS userPassword, u.su_email AS userEmail, u.su_phone AS userPhone, u.su_passport_id AS userPassportId, u.su_passport_issued_by AS userPassportIssuedBy, c.uc_name AS userCountry, u.su_blocked AS isBlocked, r.sur_role_name AS userRole, u.su_picture AS userPicture   FROM site_users u  INNER JOIN site_users_role r ON u.sur_id = r.sur_id  INNER JOIN users_countries c ON u.uc_id = c.uc_id WHERE UPPER(u.su_login) LIKE UPPER(?) ORDER BY u.su_login ASC";
	private static final String GET_USERS_SQL_STATEMENT = "SELECT u.su_login AS userLogin, u.su_surname AS userSurname, u.su_Name AS userName, u.su_password AS userPassword, u.su_email AS userEmail, u.su_phone AS userPhone, u.su_passport_id AS userPassportId, u.su_passport_issued_by AS userPassportIssuedBy, c.uc_name AS userCountry, u.su_blocked AS isBlocked, r.sur_role_name AS userRole, u.su_picture AS userPicture   FROM site_users u  INNER JOIN site_users_role r ON u.sur_id = r.sur_id  INNER JOIN users_countries c ON u.uc_id = c.uc_id ORDER BY u.su_login ASC";
	...
	private static final String GET_USER_LOTS_SQL_STATEMENT = "SELECT l_id AS lotId, l_name AS lotName, l_description AS lotDescription, l_quantity AS lotQuantity, l_picture AS lotPicture, l_date_added AS lotDateAdded, l_type AS lotType, l_status AS lotStatus, su_owner_login AS lotOwner FROM lots WHERE su_owner_login=? ORDER BY l_date_added DESC";
	private static final String GET_USER_AUCTION_PARTICIPATION_SQL_STATEMENT = "SELECT l.l_id AS lotId, l.l_name AS lotName, l.l_description AS lotDescription, l.l_quantity AS lotQuantity, l.l_picture AS lotPicture, l.l_date_added AS lotDateAdded, l_type AS lotType, l_status AS lotStatus, l.su_owner_login AS lotOwner, a.a_id AS auctionId, a.a_last_bet_time AS auctionLastBetTime, a.a_start_time AS auctionStartTime, a.a_end_time AS auctionEndTime, a.a_status AS auctionStatus, l.l_type AS lotType, l.l_status AS lotStatus, a.a_minimum_price AS auctionMinimumPrice, a.a_current_price AS auctionCurrentPrice, au_t.at_type_name AS auctionType, a.su_login_last_bet AS auctionLastBetLogin FROM site_users AS su INNER JOIN user_participation_in_bidding AS upib ON upib.su_login = su.su_login INNER JOIN auctions AS a ON upib.a_id = a.a_id INNER JOIN lots AS l ON l.l_id = a.l_id INNER JOIN auctions_type AS au_t ON au_t.at_id = a.auctions_type_at_id WHERE su.su_login=? ORDER BY l_date_added DESC";
	...
	private static final String GET_USER_WIN_LOTS_SQL_STATEMENT = "SELECT l.l_id AS lotId, l.l_name AS lotName, l.l_description AS lotDescription, l.l_quantity AS lotQuantity, l.l_picture AS lotPicture, l.l_date_added AS lotDateAdded, l.l_type AS lotType, l.l_status AS lotStatus, l.su_owner_login AS lotOwner FROM site_users AS su INNER JOIN user_participation_in_bidding AS upib ON upib.su_login = su.su_login INNER JOIN auctions AS a ON upib.a_id = a.a_id INNER JOIN lots AS l ON l.l_id = a.l_id WHERE su.su_login=? AND upib.upib_status='WON' AND a.a_status='COMPLETED' ORDER BY l_date_added DESC";
