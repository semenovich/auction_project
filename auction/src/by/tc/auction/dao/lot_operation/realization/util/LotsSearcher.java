...
public class LotsSearcher {
	...
	private static final String GET_LOTS_BY_TYPE_SQL_STATEMENT = "SELECT l_id AS lotId, l_locale AS lotLocale, l_name AS lotName, l_description AS lotDescription, l_quantity AS lotQuantity, l_picture AS lotPicture, l_date_added AS lotDateAdded, l_type AS lotType, l_status AS lotStatus, su_owner_login AS lotOwner FROM auction.lots WHERE l.l_type=? AND l_locale=? ORDER BY l_date_added DESC";
