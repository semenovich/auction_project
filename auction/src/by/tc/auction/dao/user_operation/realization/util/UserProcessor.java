...
public final class UserProcessor {
	...
	private static final String EDIT_USER_INFO_SQL_STATEMENT = "UPDATE auction.site_users SET su_surname=?, su_name=?, su_password=MD5(?), su_email=?, su_phone=?, su_passport_id=?, su_passport_issued_by=?, su_picture=? WHERE su_login=?";
	private static final String EDIT_USER_INFO_WIHOUT_PASSWORD_SQL_STATEMENT = "UPDATE auction.site_users SET su_surname=?, su_name=?, su_email=?, su_phone=?, su_passport_id=?, su_passport_issued_by=?, su_picture=? WHERE su_login=?";
	...
	private void fillEditWithoutPasswordPreparedStatement(PreparedStatement preparedStatement, User user)
			throws SQLException {
		preparedStatement.setString(1, user.getSurname());
		preparedStatement.setString(2, user.getName());
